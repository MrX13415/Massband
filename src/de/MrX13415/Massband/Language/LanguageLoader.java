...
public class LanguageLoader {
	...
	public static void updateLanguages(boolean force) {
		...
		if (language.update(force)) Massband.log.info(Massband.getConsoleOutputHeader() + " " + language._languageName + " language file updated ...");   
	...
	public static Language loadLanguage(String languageName) {
		...
		try {
			...
			try {
				...
				Massband.log.warning(Massband.getConsoleOutputHeader() + " Error: An error occurred while loading the language file.");
				e.printStackTrace();
			}
			...
			Massband.log.warning(Massband.getConsoleOutputHeader() + " Error: language not found: " + languageName);
	...
	public static boolean saveLanguage(Language lang) {
		...
		try {
			...
			writer.write("#" + Massband.getPluginDescriptionFile().getName() + " by " + Massband.getPluginDescriptionFile().getAuthors() + "\n");
			writer.write("#Language: " + lang._languageName + "  Autor: " + Massband.getPluginDescriptionFile().getAuthors() + "\n");
			writer.write("#\n");
			writer.write("# Colors: \n");
			writer.write("# \n");
			...
			for(Field langField:Language.class.getDeclaredFields()){
				if (!langField.getName().startsWith("_")){  
					...
					if(value != null) writer.write(String.format(fileFormat_keys, langField.getName(), value.replaceAll("\\n", "\\\\n")) + "\n");
				}
			}
			
			writer.close();
			
			Massband.log.info(Massband.getConsoleOutputHeader() + " Language saved: " + lang._languageName);
			
			return true;
		} catch (Exception e1) {
			Massband.log.warning(Massband.getConsoleOutputHeader() + " Error: can't create language file: " + lang._languageName);
		}finally{
			try {
